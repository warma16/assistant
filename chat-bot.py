...
import confim.show as s
import time
import os
import confim.check as check
import platform
...
def healtht(x):
    while 1:
        ...
        if datetime.datetime.now().hour == 9+12+8-24:
            ...
            playit(os.getcwd()+'/ring/ring.mp3') 
def healthr(x):   
        for i in range(0,25):
            time.sleep(60)
        make_voice('该休息了，同志。舒活舒活筋骨，抖擞抖擞精神。',show=True)
        playit(os.getcwd()+'/ring/rest.mp3')
        for i in range(0,5):
            time.sleep(60)
...
while 1:
    ij=easygui.enterbox('输入你想说的内容: ')
    if ij == '再见' or ij == '退出' :
        make_voice('再见。',show=True)
        exit(0)
    elif '升级' in ij or 'update' in ij:
        os.system('python ./confim/update.py')
